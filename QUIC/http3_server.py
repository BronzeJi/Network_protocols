'''
An example of https sever based on quic
Execution Flow
1. Command-Line Parsing: 
    Parse command-line arguments to get the configuration details.
2. Application Loading: 
    Dynamically import and load the ASGI application.
3. Server Setup: 
    Configure QUIC settings and start the server using asyncio.run.
4. Event Handling: 
    Handle QUIC events and dispatch them to the appropriate protocol handlers (HTTP, WebSocket, or WebTransport).
...
import aioquic
import wsproto # A library for handling WebSocket protocol
...
try:
    import uvloop #A high-performance event loop for asyncio (optional)
except ImportError:
    uvloop = None
...
Queues HTTP events and interacts with the ASGI application.
Sends responses back to the client, handling headers, data, and HTTP push.
''' 
...
Queues WebSocket events, processes them, and sends responses.
Handles WebSocket-specific messages like connect, send, close, and receive.
'''
...
'''
Manages WebTransport connections, a protocol built on top of HTTP/3.
Queues WebTransport events and interacts with the ASGI application.
'''
...
'''
Inherits from QuicConnectionProtocol to handle QUIC connection events.
Manages HTTP, WebSocket, and WebTransport connections based on the ALPN (Application-Layer Protocol Negotiation) protocol selected during connection setup.
Dispatches incoming QUIC events to the appropriate handler (HTTP, WebSocket, or WebTransport).
'''
